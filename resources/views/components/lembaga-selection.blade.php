<div class="lembaga-block_{{$uid}}">
  <div class="row mb-6">
    <label class="col-form-label required fw-semibold fs-6 {{ $layout === 'horizontal'? 'col-lg-4' : '' }}">Lembaga Induk</label>
    <div class="{{ $layout === 'horizontal'? 'col-lg-8' : '' }}">
      <div class="row">
        <div class="col-12 fv-row">
          <select name="{{ $lembagaIndukName }}" id="{{ $lembagaIndukName }}" aria-label="Pilih Lembaga Induk" {{ $withSelect2 ? 'data-control=select2' : '' }}
                  data-placeholder="Pilih Lembaga Induk" class="form-select form-select-solid form-select-lg" @disabled($viewOnly) >
            <option value="">Pilih Lembaga Induk</option>
            @foreach($lembagaInduk as $l)
              <option
                value="{{ $l->id }}" @selected(old($lembagaIndukName, $initLembagaIndukId) == $l->id) >{{ $l->name }}</option>
            @endforeach
          </select>
        </div>
        @if($withAdd && !$viewOnly)
          <a class="form-text text-primary cursor-pointer add-lembaga_{{$uid}}" data-level="1">Klik disini untuk membuat Lembaga Induk baru.</a>
        @endif
      </div>
    </div>
  </div>
  <div class="row mb-6">
    <label class="col-form-label required fw-semibold fs-6 {{ $layout === 'horizontal'? 'col-lg-4' : '' }}">Pusat/Unit/Fakultas</label>
    <div class="{{ $layout === 'horizontal'? 'col-lg-8' : '' }}">
      <div class="row">
        <div class="col-12 fv-row">
          <select name="{{ $lembagaUnitName }}" id="{{ $lembagaUnitName }}" aria-label="Pilih Pusat/Unit/Fakultas" {{ $withSelect2 ? 'data-control=select2' : '' }}
                  data-placeholder="Pilih Pusat/Unit/Fakultas" class="form-select form-select-solid form-select-lg" @disabled($viewOnly) >
            <option value="">Pilih Pusat/Unit/Fakultas</option>
            @foreach($lembagaUnit as $l)
              <option
                value="{{ $l->id }}" @selected(old($lembagaUnitName, $initLembagaUnitId) == $l->id) >{{ $l->name }}</option>
            @endforeach
          </select>
        </div>
        @if($withAdd && !$viewOnly)
          <a class="form-text text-primary cursor-pointer add-lembaga_{{$uid}}" data-level="2">Klik disini untuk membuat lembaga Pusat/Unit/Fakultas baru.</a>
        @endif
      </div>
    </div>
  </div>
  <div class="row mb-6">
    <label class="col-form-label required fw-semibold fs-6 {{ $layout === 'horizontal'? 'col-lg-4' : '' }}">Direktorat/Jurusan</label>
    <div class="{{ $layout === 'horizontal'? 'col-lg-8' : '' }}">
      <div class="row">
        <div class="col-12 fv-row">
          <select name="{{ $lembagaName }}" id="{{ $lembagaName }}" aria-label="Pilih Direktorat/Jurusan" {{ $withSelect2 ? 'data-control=select2' : '' }}
                  data-placeholder="Pilih Direktorat/Jurusan" class="form-select form-select-solid form-select-lg" @disabled($viewOnly) >
            <option value="">Pilih Direktorat/Jurusan</option>
            @foreach($lembaga as $l)
              <option
                value="{{ $l->id }}" @selected(old($lembagaName, $initLembagaId) == $l->id) >{{ $l->name }}</option>
            @endforeach
          </select>
        </div>
        @if($withAdd && !$viewOnly)
          <a class="form-text text-primary cursor-pointer add-lembaga_{{$uid}}" data-level="3">Klik disini untuk membuat lembaga Direktorat/Jurusan baru.</a>
        @endif
      </div>
    </div>
  </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body" id="modal-create-lembaga-body">
          <form id='create-lembaga-form' class="form"  method="POST" action="{{ route('common.lembaga-store') }}">
            <div class="fv-row mb-6">
              <label class="col-form-label required fw-semibold fs-6">Lembaga Induk</label>
              <input type="text" id="lembaga_induk_name" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0"
                     disabled  />
            </div>
            <div class="fv-row mb-6">
              <label class="col-form-label required fw-semibold fs-6">Pusat/Unit/Fakultas</label>
              <input type="text" id="lembaga_unit_name" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0"
                     disabled  />
            </div>
            <div class="fv-row mb-6">
  ...
  <script type="text/javascript">
    ...
    KTUtil.onDOMContentLoaded(function () {
      if({{$uid}}.withAdd && !{{$uid}}.viewOnly) {
        ...
        {{$uid}}.modalCreateLembaga = new bootstrap.Modal(document.getElementById('modal-create-lembaga'));
            ...
            success: function (res) {
              switch ({{$uid}}.lembagaLevel) {
                case 1:
                  ...
                  {{$uid}}.lembagaIndukElm.append(`<option value="${res.lembaga_id}" selected>${res.lembaga_name}</option>`);
                ...
                case 2:
                  ...
                  {{$uid}}.lembagaUnitElm.append(`<option value="${res.lembaga_id}" selected>${res.lembaga_name}</option>`);
                ...
                case 3:
                  ...
                  {{$uid}}.lembagaElm.append(`<option value="${res.lembaga_id}" selected>${res.lembaga_name}</option>`);
      ...
      {{$uid}}.lembagaIndukElm.on('change', function(){
        ...
        $.ajax({
          ...
          success: function (res){
            ...
            {{$uid}}.lembagaUnitElm.append(`<option value="" selected>Pilih Pusat/Unit/Fakultas</option>`);
            {{$uid}}.lembagaElm.append(`<option value="" selected>Pilih Direktorat/Jurusan</option>`);
            $.each(res.data, function(index, item){
              {{$uid}}.lembagaUnitElm.append(`<option value="${item.id}">${item.name}</option>`);
            })
            if({{$uid}}.withSelect2){
              {{$uid}}.lembagaUnitElm.select2();
              {{$uid}}.lembagaElm.select2();
      ...
      {{$uid}}.lembagaUnitElm.on('change', function(){
        ...
        $.ajax({
          ...
          success: function (res){
            ...
            {{$uid}}.lembagaElm.append(`<option value="" selected>Pilih Direktorat/Jurusan</option>`);
            $.each(res.data, function(index, item){
              {{$uid}}.lembagaElm.append(`<option value="${item.id}">${item.name}</option>`);
            })
            if({{$uid}}.withSelect2)
              {{$uid}}.lembagaElm.select2();
      ...
      $('.add-lembaga_{{$uid}}').on('click',function() {
        ...
        let parent_id = '';
        if(level === 2){
          $('#lembaga_induk_name').val({{$uid}}.lembagaIndukElm.find(':selected').text()).parent().removeClass('d-none')
          parent_id = {{$uid}}.lembagaIndukElm.val();
        }else if(level === 3){
          $('#lembaga_induk_name').val({{$uid}}.lembagaIndukElm.find(':selected').text()).parent().removeClass('d-none')
          $('#lembaga_unit_name').val({{$uid}}.lembagaUnitElm.find(':selected').text()).parent().removeClass('d-none')
