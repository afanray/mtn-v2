<div class="address-block">
  <div class="row mb-6">
    <label class="col-form-label required fw-semibold fs-6 {{ $layout === 'horizontal'? 'col-lg-4' : '' }}">Provinsi</label>
        ...
        <div class="col-12 fv-row">
          <select name="{{ $provinceName }}" id="{{ $provinceName }}" aria-label="Pilih Provinsi"
            ...
            @foreach($provinces as $l)
              <option
                value="{{ $l->id }}" @selected(old($provinceName, $initProvinceId) == $l->id) >{{ $l->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
  ...
  <div class="row mb-6">
    <label class="col-form-label required fw-semibold fs-6 {{ $layout === 'horizontal'? 'col-lg-4' : '' }}">Kabupaten/Kota</label>
    <div class="{{ $layout === 'horizontal'? 'col-lg-8' : '' }}">
      <div class="row">
        <div class="col-12 fv-row">
          <select name="{{ $regencyName }}" id="{{ $regencyName }}" aria-label="Pilih Kabupaten/Kota"
            ...
            @foreach($regencies as $r)
              <option
                value="{{ $r->id }}" @selected(old($regencyName, $initRegencyId) == $r->id) >{{ $r->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
        ...
        $.ajax({
          ...
          success: function (res){
            ...
            $.each(res.data, function(index, item){
              regencyElm.append(`<option value="${item.id}">${item.name}</option>`);
            })
