                  ...
                  >
                    ...
                    @foreach(\App\Constants\Common::getTahun() as $t)
                      ...
                      @foreach($t['data'] as $tahun)
                          <option value="{{ $tahun }}" @selected($tahun == old('tahun',$model->year))>{{ $tahun }}</option>
                      @endforeach
                      </optgroup>
                    @endforeach
                  </select>
                  ...
                  @if($type === 'add' || ($type === 'edit' && $status === \App\Constants\Common::STATUS_NULL))
                    <div class="col-12 fv-row">
                      <input type="text" name="{{ $attr['field'] }}" id="{{ $attr['field']  }}"
                             ...
                             placeholder="{{ $attr['label'] }}" value="{{ old($attr['field'], $model->{$attr['field']}) }}" />
                    </div>
                  @else
                      ...
                      @if((\App\Models\User::isPic() || \App\Models\User::isSuperAdmin()) && isset($dataInputs[$attr['field']]))
                        @if($dataInputs[$attr['field']][0]['user'])
                          <div class="mb-3 d-flex flex-column">
                            <div class="form-text text-muted">disunting oleh: <span class="text-primary">{{$dataInputs[$attr['field']][0]['user']['name']}}</span></div>
                            <div class="form-text text-muted">disunting pada: <span class="text-primary">{{date('Y-m-d H:i:s', strtotime($dataInputs[$attr['field']][0]['created_at']))}}</span></div>
                          </div>
                        @endif
                      @endif
                    </div>
                    <div class="col-4 fv-row">
                      ...
                      >
                        ...
                        <option value="1" {{ isset($dataInputs[$attr['field']]) ? ($dataInputs[$attr['field']][0]['status'] == 1 ? 'selected' : '') : '' }}>Valid</option>
                        <option value="2" {{ isset($dataInputs[$attr['field']]) ? ($dataInputs[$attr['field']][0]['status'] == 2 ? 'selected' : '') : '' }}>Ditolak</option>
                      </select>
                    </div>
                    <div class="col-4 fv-row">
                      <textarea
                        ...
                        {{ \App\Models\User::isOperator() ? 'disabled' : '' }}
