  ...
  <div class="card ">
    <div class="card-header card-header-stretch">
      <div class="d-flex py-3 gap-2">
        <h3 class="card-title">Capaian Indikator</h3>
        <select class="form-select" id="change-year">
          @foreach(\App\Constants\Common::getTahun() as $t)
              <option value="{{ $loop->index }}" @selected($loop->index == $yearIndex)>{{ $t['label'] }} ({{ min($t['data']) }} - {{ max($t['data']) }})</option>
          @endforeach
        </select>
      </div>
          ...
          <div class="card">
                ...
                <div class="d-flex justify-content-lg-between flex-column flex-lg-row w-100">
                  ...
                  <div class="d-flex align-items-center position-relative my-1">
                    @if(\App\Models\User::isSuperAdmin())
                      <button type="button" class="btn btn-success save-target save-target-riset me-3" data-type="riset" style="display: none">
                        <i class="fa fa-save"></i> <span>Simpan</span>
                      </button>
                      <a href="javascript:void(0)" class="btn btn-info edit-target me-3" data-type="riset" data-edit="0">
            ...
            <div class="card-body p-0">
              <form id="form-target-riset" action="{{ route('capaian-indikator.save-target') }}">
                ...
                <table class="table table-striped table-bordered fs-6 gy-5 gx-5" id="table-riset">
                  <thead>
                      ...
                      <tr>
                        ...
                        @foreach($activeYearGroup['data'] as $tahun)
                          <td class="text-center">
                            <span class="d-inline-block mb-3">{{ isset($data[$tahun]) ? ($data[$tahun][0][$ind['field']] ? number_format($data[$tahun][0][$ind['field']], 2, ',', '.') : 'Dalam Perhitungan') : 'Dalam Perhitungan' }}</span>
                            <div class="progress">
                              <div class="progress-bar progress-bar-striped bg-success" role="progressbar" aria-label="Example with label" style="width: {{ \App\Constants\Common::getPercentageTarget($targetRiset[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}%;" aria-valuenow="{{ \App\Constants\Common::getPercentageTarget($targetRiset[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}" aria-valuemin="0" aria-valuemax="100"></div>
                              <span class="progress-label">{{ \App\Constants\Common::getPercentageTarget($targetRiset[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}%</span>
                            </div>
                          </td>
                          <td class="text-center">
                            <span class="text-target" id="text-target-{{ $targetRiset[$ind['field']][$tahun][0]['id'] }}">{{ $targetRiset[$ind['field']][$tahun][0]['target'] ?? 'N/A' }}</span>
                            <input type="number" name="target[{{ $targetRiset[$ind['field']][$tahun][0]['id'] }}]" value="{{ $targetRiset[$ind['field']][$tahun][0]['target'] }}" class="form-control input-target" style="display: none">
                          </td>
                        @endforeach
                        <td>N/A</td>
                        <td>
                          <a href="javascript:void(0)" class="fw-bold open-chart" data-id="{{ $ind['id'] }}">Lihat Grafik</a>
                        </td>
                      </tr>
                    @endforeach
                  @endforeach
            ...
            <div class="card-header border-0 p-0">
              ...
              <div class="card-title w-100">
                <div class="d-flex justify-content-lg-between flex-column flex-lg-row w-100">
                  ...
                  <div class="d-flex align-items-center position-relative my-1">
                    @if(\App\Models\User::isSuperAdmin())
                      <button type="button" class="btn btn-success save-target save-target-seni me-3" data-type="seni" style="display: none">
                        <i class="fa fa-save"></i> <span>Simpan</span>
                      </button>
                      <a href="javascript:void(0)" class="btn btn-info edit-target me-3" data-type="seni" data-edit="0">
                        <i class="fa fa-pencil-alt"></i> <span>Sunting Target</span>
                      </a>
                    @endif
                    <a href="javascript:void(0)" class="btn btn-primary">
            ...
            <div class="card-body p-0">
              <div class="table-responsive">
                <form id="form-target-seni" action="{{ route('capaian-indikator.save-target') }}">
                  <table class="table table-striped table-bordered fs-6 gy-5 gx-5" id="table-seni">
                  <thead>
                  <tr class="text-start text-gray-700 fw-bold fs-7 text-uppercase gs-0">
                      ...
                      <tr>
                        ...
                        @foreach($activeYearGroup['data'] as $tahun)
                          <td class="text-center">
                            <span class="d-inline-block mb-3">{{ isset($data[$tahun]) ? ($data[$tahun][0][$ind['field']] ? number_format($data[$tahun][0][$ind['field']], 2, ',', '.') : 'Dalam Perhitungan') : 'Dalam Perhitungan' }}</span>
                            <div class="progress">
                              <div class="progress-bar progress-bar-striped bg-success" role="progressbar" aria-label="Example with label" style="width: {{ \App\Constants\Common::getPercentageTarget($targetSeni[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}%;" aria-valuenow="{{ \App\Constants\Common::getPercentageTarget($targetSeni[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}" aria-valuemin="0" aria-valuemax="100"></div>
                              <span class="progress-label">{{ \App\Constants\Common::getPercentageTarget($targetSeni[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}%</span>
                            </div>
                          </td>

                          <td class="text-center">
                            <span class="text-target" id="text-target-{{ $targetSeni[$ind['field']][$tahun][0]['id'] }}">{{ $targetSeni[$ind['field']][$tahun][0]['target'] ?? 'N/A' }}</span>
                            <input type="number" name="target[{{ $targetSeni[$ind['field']][$tahun][0]['id'] }}]" value="{{ $targetSeni[$ind['field']][$tahun][0]['target'] }}" class="form-control input-target" style="display: none">
                          </td>
                        @endforeach
                        <td>N/A</td>
                        <td>
            ...
            <div class="card-header border-0 p-0">
              ...
              <div class="card-title w-100">
                <div class="d-flex justify-content-lg-between flex-column flex-lg-row w-100">
                  ...
                  <div class="d-flex align-items-center position-relative my-1">
                    @if(\App\Models\User::isSuperAdmin())
                      <button type="button" class="btn btn-success save-target save-target-or me-3" data-type="or" style="display: none">
                        <i class="fa fa-save"></i> <span>Simpan</span>
                      </button>
                      <a href="javascript:void(0)" class="btn btn-info edit-target me-3" data-type="or" data-edit="0">
                        <i class="fa fa-pencil-alt"></i> <span>Sunting Target</span>
                      </a>
                    @endif
                    <a href="javascript:void(0)" class="btn btn-primary">
            ...
            <div class="card-body p-0">
              <div class="table-responsive">
                <form action="{{ route('capaian-indikator.save-target') }}" id="form-target-or">
                  <table class="table table-striped table-bordered fs-6 gy-5 gx-5" id="table-or">
                  <thead>
                  <tr class="text-start text-gray-700 fw-bold fs-7 text-uppercase gs-0">
                      ...
                      <tr>
                        ...
                        <td>
                          @if(is_array($ind['min']))
                            @foreach($ind['min'] as $min)
                              ...
                              @else
                                <span class="ml-2">{{ $ind['min_satuan'][$loop->index] }}</span>
                              @endif
                            @endforeach
                          @else
                            {{ is_int($ind['min']) ? number_format($ind['min'], 0, ',', '.') : $ind['min'] }}
                        ...
                        @foreach($activeYearGroup['data'] as $tahun)
                          <td class="text-center">
                            <span class="d-inline-block mb-3">{{ isset($data[$tahun]) ? ($data[$tahun][0][$ind['field']] ? number_format($data[$tahun][0][$ind['field']], 2, ',', '.') : 'Dalam Perhitungan') : 'Dalam Perhitungan' }}</span>
                            <div class="progress">
                              <div class="progress-bar progress-bar-striped bg-success" role="progressbar" aria-label="Example with label" style="width: {{ \App\Constants\Common::getPercentageTarget($targetOr[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}%;" aria-valuenow="{{ \App\Constants\Common::getPercentageTarget($targetOr[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}" aria-valuemin="0" aria-valuemax="100"></div>
                              <span class="progress-label">{{ \App\Constants\Common::getPercentageTarget($targetOr[$ind['field']][$tahun][0]['target'] ?? 0, $data[$tahun][0][$ind['field']] ?? 0) }}%</span>
                            </div>
                          </td>
                            <td class="text-center">
                              <span class="text-target" id="text-target-{{ $targetOr[$ind['field']][$tahun][0]['id'] }}">{{ $targetOr[$ind['field']][$tahun][0]['target'] ?? 'N/A' }}</span>
                              <input type="number" name="target[{{ $targetOr[$ind['field']][$tahun][0]['id'] }}]" value="{{ $targetOr[$ind['field']][$tahun][0]['target'] }}" class="form-control input-target" style="display: none">
                            </td>
                        @endforeach
                        <td>N/A</td>
                        <td>
...
@section('css')
  <style>
    ...
    #table-seni.table-striped tbody tr:nth-of-type(odd){
      background-color: #FBE3D5!important;
    }
