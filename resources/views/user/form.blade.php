          ...
          <div @class([
            ...
            <div class="col-lg-8">
              <div class="row">
                <div class="col-12 fv-row">
                  @foreach($inputs as $i)
                    <div class="form-check mb-5 form-check-{{ $i->bidang_id }}" style="display: {{ $model->id && $model->bidang_id === $i->bidang_id ? 'block':'none' }}">
                      <input class="form-check-input check-{{ $i->bidang_id }}" type="checkbox" name="inputs[{{ $i->bidang_id }}][]" value="{{ $i->id }}"
