      ...
      <div class="banner-inner">
        ...
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb wow fadeInUp delay-0-4s" style="visibility: visible; animation-name: fadeInUp;">
            ...
            <li class="breadcrumb-item active">Ajang Talenta</li>
          </ol>
        </nav>
      </div>
    </div>
      ...
      <div class="row">
        <div class="col-12">
          <ul class="project-filter mb-55">
            <li @class(['current'=>!$bidang_id])><a href="{{ route('home.ajang-talenta') }}">Semua</a></li>
            @foreach($bidang as $b)
              <li @class(['current'=>$b->id == $bidang_id])><a href="{{ route('home.ajang-talenta',['_b'=>$b->id]) }}">{{ $b->name }}</a></li>
            @endforeach
          </ul>
        </div>
      </div>
