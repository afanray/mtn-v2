...
@section('body')
  <div class="row">
    ...
    <div class="col-12 col-md-6 col-xl-4 mb-5">
      <!--begin::Card widget 3-->
      <div class="card card-flush bgi-no-repeat bgi-size-contain bgi-position-x-end h-xl-100" style="background-color: #50cd89;background-image:url('/assets/media/svg/shapes/wave-bg-dark.svg')">
        <!--begin::Header-->
        <div class="card-header pt-5 mb-3">
          <!--begin::Icon-->
          <div class="d-flex flex-center rounded-circle h-80px w-80px" style="border: 1px dashed rgba(255, 255, 255, 0.4);background-color: #50cd89">
            <i class="fa fa-users text-white fs-2qx lh-0"></i>
          </div>
          <!--end::Icon-->
        </div>
  ...
  <div class="card mt-5">
    <div class="card-header card-header-stretch">
      <div class="d-flex py-3 gap-2">
        <h3 class="card-title">Daftar Periksa Kelengkapan Data Indikator</h3>
      ...
      <div class="card-toolbar">
        <ul class="nav nav-tabs nav-line-tabs nav-stretch fs-6 border-0">
          @foreach($bidang as $k=>$v)
            <li class="nav-item">
              <a class="nav-link fw-bolder text-active-dark {{ $loop->index === 0 ? 'active':'' }}" data-bs-toggle="tab" href="#kt_tab_pane_{{$k}}">{{ $v['label'] }}</a>
            </li>
          @endforeach
        </ul>
      </div>
    ...
    <div class="card-body">
      <div class="tab-content" id="myTabContent">
        @foreach($bidang as $k=>$v)
          <div class="tab-pane fade {{ $loop->index === 0 ? 'show active':'' }}" id="kt_tab_pane_{{$k}}" role="tabpanel">
            <div class="table-responsive">
              <table class="table table-bordered table-striped fs-6 gy-5 gx-5" >
                <thead>
                <tr class="text-start text-gray-700 fw-bold fs-7 text-uppercase gs-0">
                  @if(!\App\Models\User::isOperator())
                    <th class="min-w-125px align-middle bg-primary text-white" rowspan="2">Operator</th>
                  @endif
                  <th class="min-w-125px align-middle bg-primary text-white" rowspan="2">Indikator</th>
                  @foreach($activeYearGroup['data'] as $tahun)
                    <th class="min-w-125px text-center bg-gray-300i text-dark fw-bolder" colspan="3">{{ $tahun }}</th>
                  @endforeach
                </tr>
                <tr class="text-start text-gray-700 fw-bold fs-7 text-uppercase gs-0">
                  @foreach($activeYearGroup['data'] as $tahun)
                    <th class="min-w-125px text-center bg-gray-300i text-dark fw-bolder">Status</th>
                    <th class="min-w-125px text-center bg-gray-300i text-dark fw-bolder">Komentar</th>
                    <th class="min-w-125px text-center bg-gray-300i text-dark fw-bolder">Update</th>
                  @endforeach
                </tr>
                </thead>
                ...
                @foreach($v['attributes'] as $attr)
                  <tr>
                    ...
                    @foreach($activeYearGroup['data'] as $tahun)
                        ...
                        <td>
                          @if(isset($v['dataInputs'][$tahun][$attr['field']]))
                            @switch($v['dataInputs'][$tahun][$attr['field']][0]['status'])
                              ...
                              @default
                                <span class="badge badge-lg badge-warning">Perlu Validasi</span>
                            @endswitch
                          @else
                            <span class="badge badge-lg badge-primary">Belum Di-input</span>
                          @endif
                        </td>
                        <td>{{ isset($v['dataInputs'][$tahun][$attr['field']]) ? $v['dataInputs'][$tahun][$attr['field']][0]['comment'] : '-' }}</td>
                        <td>
                          @if(isset($v['dataInputs'][$tahun][$attr['field']]))
                      ...
                      @else
                        <td><span class="badge badge-lg badge-primary">Belum Di-input</span></td>
                        <td>-</td>
                        <td>-</td>
                      @endif
                    @endforeach
