...
class DataDasarController extends Controller
{
  ...
  public function store(Request $request): RedirectResponse
  {
    ...
    if ((User::isPic() || User::isSuperAdmin()) && $request->input('data_input') && $type === 'edit') {
      ...
      foreach ($dataInputPost as $inpId => $inpAttr) {
        ...
        if (empty($checkInputData)) {
          ...
          $checkInputData->url = $inpId;
          $checkInputData->bottleneck = $inpId;
          $checkInputData->debottleneck = $inpId;
        }
    ...
    if ((User::isOperator()) && $type === 'add') {
      ...
      foreach ($attributes as $attr) {
        if ($bidang_id === 1) {
          $dataInput[] = new DataDasarInputStatus(
            [
              'input_data_id' => $attr['id'],
              'url' => $request->input($attr['field'] . '_validation_url'),
              'bottleneck' => $request->input($attr['field'] . '_bottleneck'),
              'debottleneck' => $request->input($attr['field'] . '_debottleneck')
            ]
          );
          // @dd($dataInput);
        } else if ($bidang_id === 2) {
          $dataInput[] = new DataDasarSbInputStatus(
            [
              'input_data_id' => $attr['id'],
              'url' => $request->input($attr['field'] . '_validation_url'),
              'bottleneck' => $request->input($attr['field'] . '_bottleneck'),
          ...
          $dataInput[] = new DataDasarOrInputStatus(
            [
              'input_data_id' => $attr['id'],
              'url' => $request->input($attr['field'] . '_validation_url'),
              'bottleneck' => $request->input($attr['field'] . '_bottleneck'),
